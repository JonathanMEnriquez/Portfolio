import React, {useState, useEffect} from 'react';
import { Link } from 'react-router-dom';
import ContentJSON from '../json/content.json';
import Gallery from './Gallery';
import dojo from '../images/coding-dojo.png';
import '../css/About.css';

const ABOUT_CATEGORIES = {
    MAIN: 'main',
    APPROACH: 'approach',
    MOST: 'enjoy_most',
    HOBBIES: 'hobbies'
};

const About = ({ lang }) => {
    const text = ContentJSON.about[lang || 'en'];
    const [display, setDisplay] = useState(text.story.main);
    const [displayCat, setDisplayCat] = useState(ABOUT_CATEGORIES.MAIN);

    useEffect(() => {
        if (text && displayCat) {
            setDisplay(text.story[displayCat]);
        }
    }, [displayCat, text]);
    
    return (
        <div className="about">
            <section className="info">
                <h1>{text.header}</h1>
                <ul className="topic-nav">
                    <li onClick={() => setDisplayCat(ABOUT_CATEGORIES.MAIN)}
                        className={display ===ABOUT_CATEGORIES.MAIN ? 'selected' : ''}>
                       Intro
                    </li>
                    <li>|</li>
                    <li onClick={() => setDisplayCat(ABOUT_CATEGORIES.APPROACH)}
                        className={display === ABOUT_CATEGORIES.APPROACH ? 'selected' : ''}>
                       Work Approach
                    </li>
                    <li>|</li>
                    <li onClick={() => setDisplayCat(ABOUT_CATEGORIES.MOST)}
                        className={display === ABOUT_CATEGORIES.MOST ? 'selected' : ''}>
                       Favorite Part of Coding
                    </li>
                    <li>|</li>
                    <li onClick={() => setDisplayCat(ABOUT_CATEGORIES.HOBBIES)}
                        className={display === ABOUT_CATEGORIES.HOBBIES ? 'selected' : ''}>
                       Hobbies
                    </li>
                </ul>
                <div className="about-body">
                    {display && display.map((par, key) => (
                        <p key={key}>{par}</p>
                    ))}
                </div>
                <footer>
                    <p className="contact-me">{text.story.main_3}&nbsp;
                        <Link to="contact">
                            {text.story.contact}
                        </Link>
                    </p>
                    <div className="schools">
                        <svg xmlns="http://www.w3.org/2000/svg" id="institutions" 
                            width="120" height="100" viewBox="0 0 192.756 192.756">
                            <defs>
                            </defs>
                            <path className="cls-1" d="M33.08 78.09h-5.79l-.53-3c-2.36 2.66-6.41 3.59-9.6 3.59C6.75 78.67 0 72.3 0 64.31S6.75 50 17.17 50c7.14 0 14.76 3 15.58 10h-9c-.29-2.35-3.18-3.78-6.8-3.78-5.79 0-8 4.09-8 8.18s2.22 8.18 8 8.18c4.44 0 7.43-1.66 7.91-4.17h-6.25V63h14.47v15.09zm9.97-8.41c.15 2.74 2.12 4.55 5.59 4.55 2 0 3.91-.73 4.68-2.2H61c-1.49 4.63-6.85 6.64-12.44 6.64-8.15 0-13.69-3.94-13.69-10.65 0-6.13 6.12-10.53 13.5-10.53 9 0 13.45 5.4 13.07 12.19H43.05zM53.23 66a4.6 4.6 0 0 0-4.87-4.09A5 5 0 0 0 43.05 66h10.18zm22.64-8.51c7.76 0 13.21 4.59 13.21 10.61s-5.45 10.57-13.21 10.57-13.16-4.56-13.16-10.58 5.4-10.6 13.16-10.6zm0 16.16c3.81 0 5-3 5-5.56s-1.21-5.59-5-5.59-5 3-5 5.59 1.19 5.56 5 5.56zm14.57-15.59h8v3.51h.1c1.21-2.55 4-3.9 7.52-3.9a10.86 10.86 0 0 1 1.93.15v6a14.1 14.1 0 0 0-3.13-.39c-4.05 0-6.17 2.24-6.17 4.75v9.91h-8.2v-20zm43.65 17.6c0 3.28-.77 9.45-12.49 9.45-5.84 0-11.33-1-12.3-6.4h8.2a2.19 2.19 0 0 0 1.4 1.74 8 8 0 0 0 3 .42c3.91 0 4.29-2.2 4.29-4.67v-1.89h-.1a8.55 8.55 0 0 1-6.65 2.93c-7.57 0-11-4.32-11-10 0-5.32 3.91-9.8 11-9.8 3 0 5.26 1 6.7 3.09h.1v-2.47h7.86v17.6zM121.41 62.5c-3.62 0-4.77 2.54-4.77 5a4.4 4.4 0 0 0 4.73 4.7c3.47 0 5-2.16 5-4.67s-1.3-5.03-4.96-5.03zm15.81-4.44h8.2v20h-8.2v-20zm8.2-2.51h-8.2v-5h8.2v5zm2.4 8.69a6.11 6.11 0 0 1 3.91-5.36 20 20 0 0 1 8-1.39c5.93 0 11.67 1 11.67 6.71V73a9.55 9.55 0 0 0 1 5.13h-8.25a6.49 6.49 0 0 1-.48-1.89c-2.12 1.77-5.26 2.47-8.25 2.47-4.77 0-8.53-1.93-8.53-6.06 0-6.52 8.87-6 14.56-6.94 1.4-.23 2.17-.62 2.17-1.85 0-1.51-2.27-2.08-4-2.08-2.27 0-3.71.81-4.1 2.51h-7.67zm10.61 10.38c3.91 0 5.21-1.77 5-5.9-1.16.58-3.28.69-5.06 1.08s-3.38 1-3.38 2.55 1.61 2.26 3.44 2.26zM97.86 89.26H87.59v21.37h-9V89.26H68.35v-6.17h29.51v6.17z"></path>
                            <path className="cls-1" d="M98.82 102.23c.14 2.74 2.12 4.55 5.59 4.55 2 0 3.91-.73 4.68-2.2h7.67c-1.49 4.63-6.85 6.64-12.44 6.64-8.15 0-13.69-3.94-13.69-10.65 0-6.13 6.12-10.53 13.5-10.53 9 0 13.45 5.4 13.07 12.19H98.82zM109 98.56a4.6 4.6 0 0 0-4.87-4.09 5 5 0 0 0-5.3 4.09H109zm27.52-.62c0-1.7-1.93-2.89-4-2.89-4.58 0-5.3 3.13-5.3 5.91 0 2.58 1.4 5.25 4.87 5.25 2.84 0 4.29-1.31 4.68-3.43h8c-.72 5.36-5.93 8.45-12.59 8.45-7.52 0-13.12-4.13-13.12-10.26C119 94.59 124.07 90 132.13 90c6.22 0 11.77 2.62 12.34 7.91h-8zm9.87-14.85h8.2v10.26h.1c1.25-2 4.63-3.32 7.62-3.32 8.34 0 9 4.86 9 7.75v12.85h-8.2V101c0-2.74.39-5.52-4.1-5.52a4.08 4.08 0 0 0-4.44 4.28v10.92h-8.2V83.09z"></path>
                            <path className="logo" d="M182.31 83.46L190.8 115.3 182.31 115.3 182.31 83.46 182.31 83.46z"></path>
                            <path className="logo" d="M199.29 83.46L190.8 115.3 199.29 115.3 199.29 83.46 199.29 83.46z"></path>
                            <path className="logo" d="M195.04 99.05L190.8 35.37 186.56 99.05"></path>
                            <path className="logo" d="M182.31 119.14H199.29V123.54H182.31z"></path>
                            <path className="cls-1" d="M206.75 117.42a3.61 3.61 0 1 1-3.63 3.59 3.59 3.59 0 0 1 3.63-3.59zm0 6.68a3.08 3.08 0 1 0-3-3.09 3 3 0 0 0 3 3.09zm-1.41-5.19H207c1 0 1.47.4 1.47 1.21a1.08 1.08 0 0 1-1.11 1.15l1.21 1.86h-.7l-1.15-1.81H206v1.81h-.65v-4.21zm.65 1.87h.68c.58 0 1.11 0 1.11-.69s-.48-.65-.93-.65H206v1.34z"></path>
                        </svg>
                        <svg xmlns="http://www.w3.org/2000/svg" 
                            width="60" height="60" viewBox="0 0 192.756 192.756">
                            <g fillRule="evenodd" clipRule="evenodd">
                            <path fill="none" d="M0 0h192.756v192.756H0V0z"/>
                            <path d="M96.484 176.709c-44.356 0-80.436-36.08-80.436-80.438 0-44.355 36.08-80.223 80.436-80.223 44.355 0 80.223 35.868 80.223 80.223 0 44.358-35.867 80.438-80.223 80.438z"/>
                            <path fill="#f6d43c" d="M51.066 124.711V74.624h21.648v35.655h29.501v14.432H51.066z"/>
                            <path d="M123.65 124.711h-13.584l6.58-19.312 7.004 19.312zm-22.072 19.738l4.244-9.551h21.436l4.457 9.551h21.436l-25.469-54.332h-21.859l-25.255 54.332h21.01zM62.739 48.944c-.212-2.334-1.698-3.607-4.032-3.607-2.547 0-4.033 2.122-4.033 6.367 0 3.184.637 6.155 4.457 6.155 2.334 0 3.396-1.698 3.608-3.82h7.216c-.849 5.73-4.881 9.338-10.824 9.338-7.428 0-11.885-4.456-11.885-11.885 0-7.216 4.882-11.672 11.249-11.672 6.792 0 10.611 3.396 11.248 9.125h-7.004v-.001zM82.689 59.556c-1.273 0-1.91-.637-1.91-1.274 0-2.122 2.759-1.698 4.881-2.759 0 2.335-.637 4.033-2.971 4.033zm10.187 3.183v-.212c-.85-.85-.85-1.274-.85-2.123v-7.64c0-3.821-.424-6.792-8.064-6.792-6.58 0-8.914 2.122-8.914 5.518h6.155c.212-1.485 1.485-1.697 2.334-1.697.636 0 2.122.212 2.122 1.485 0 2.971-11.673-.424-11.673 7.003 0 3.396 2.759 4.881 5.731 4.881 2.546 0 4.456-.424 6.154-2.123l.424 1.698h6.581v.002zM98.395 40.455h6.367v22.284h-6.367V40.455zM87.783 74.412c0-.637-.425-.849-.637-1.062-.425-.212-.849-.424-1.486-.424-1.061 0-1.698.424-1.698 1.062 0 2.334 9.338.849 9.338 6.792 0 3.608-3.184 5.518-7.641 5.518-4.245 0-7.003-2.547-7.003-5.518h5.093c0 .636.424 1.061.849 1.273.424.425.849.425 1.485.425 1.273 0 2.123-.425 2.123-1.062 0-2.547-9.338-.849-9.338-7.003 0-3.396 2.972-5.093 7.004-5.093 4.457 0 6.579 2.334 7.004 5.093h-5.093v-.001zM105.186 85.873c-6.154.212-7.004-.212-7.004-4.245v-4.881h-1.697v-2.759h1.91V70.38h4.457v3.608h2.334v2.759h-2.334v4.245c0 1.061.211 1.486 1.484 1.486h.85v3.395zM115.373 83.539c-1.062 0-1.486-.425-1.486-1.062 0-1.486 1.91-1.062 3.396-1.91.213 1.698-.424 2.972-1.91 2.972zm7.217 2.334v-.213c-.639-.637-.639-.849-.639-1.485v-5.518c0-2.759-.211-4.882-5.729-4.882-4.67 0-6.58 1.486-6.58 3.821h4.457c.211-.849 1.061-1.062 1.697-1.062.426 0 1.486.212 1.486 1.062 0 2.123-8.277-.213-8.277 5.094 0 2.334 1.91 3.395 4.244 3.395 1.697 0 2.973-.212 4.246-1.485l.424 1.273h4.671zM134.475 85.873c-6.156.212-6.793-.212-6.793-4.245v-4.881h-1.697v-2.759h1.697V70.38h4.457v3.608h2.336v2.759h-2.336v4.245c0 1.061.213 1.486 1.486 1.486h.85v3.395zM143.176 78.657c0-1.274.637-2.123 2.121-2.123 1.062 0 2.123 1.062 2.123 2.123h-4.244zm8.914 2.334c0-4.245-2.547-7.216-7.004-7.216-3.82 0-6.58 2.335-6.58 6.368 0 3.82 2.971 5.942 6.58 5.942 2.758 0 5.73-.849 6.578-3.82h-4.457c-.424.637-1.061 1.061-1.697 1.061-1.273 0-2.334-.849-2.334-2.334h8.914v-.001z" fill="#f6d43c"/>
                            </g>
                        </svg>
                        <img src={dojo} alt="dojo" />
                    </div>
                </footer>
            </section>
            <section className="pics">
                <Gallery content={text.imageInfo} />
            </section>
        </div>
    );
}

export default About;
